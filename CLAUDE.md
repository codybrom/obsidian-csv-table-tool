...
- `src/csv-view.ts` — `CSVView extends FileView`. On `onLoadFile`, runs `detectSeparator()` and `detectQuote()` against the file content, registers a `vault.on('modify')` listener (via `registerEvent` so cleanup is automatic) to re-render on external file changes, then calls `render()`. State (`ParseOptions { separator, quote }` and `treatFirstRowAsHeader`) lives on the view instance and is _not_ persisted. The bottom format bar (`renderFormatBar`) provides segmented `role="radiogroup"` controls for Separator and Quote plus a Header checkbox; changing any control mutates view state and triggers a re-render. The table includes a leftmost row-index column and right-aligns columns flagged numeric by `detectNumericColumns()`.
- `src/parser.ts` — Hand-rolled CSV parser (state machine over characters). Takes `ParseOptions` for `separator` (any single char) and `quote` (`"double"` / `"backslash"` / `"none"`). Handles embedded commas, embedded newlines inside quotes, escaped `""` (or `\"`), and both `\n` / `\r\n` line endings. Also exports three detection helpers: `detectSeparator(text)` (variance-weighted heuristic over the first 10 lines, scoring `,` `;` `\t` `|`), `detectQuote(text)` (first-8K-byte counts of `"` vs `\"`), and `detectNumericColumns(rows, hasHeader)` (column flagged numeric when ≥3 non-empty cells and ≥80% match a numeric regex). No third-party CSV library.
